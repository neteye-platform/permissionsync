...
Scope = TypedDict(
    "Scope",
    {"scope-type": str, "id": str, "propagation": str},
)
Constraint = TypedDict(
    "Constraint",
    {"constraint-type": str, "values": list[str]},
)


class Assignment(TypedDict):
    """One assignment in a provider document."""
...
class ProviderDocument(TypedDict):
    """The complete fixture provider-document shape."""
    ...
    "constraints": [
        {
            ...
            "values": ["collector-a", "collector-b"],
        },
...
DESCENDANTS_DOC: ProviderDocument = copy.deepcopy(VALID_DOC)
descendants_scope = DESCENDANTS_DOC["assignments"][0]["scope"]
if descendants_scope is not None:
    descendants_scope["propagation"] = "descendants"

UNSUPPORTED_ROLE_DOC: ProviderDocument = {
    "version": 1,
    "assignments": [{"kind": "role", "id": "super-user", "scope": None}],
    "constraints": [],
}

UNSUPPORTED_VERSION_DOC: ProviderDocument = {
    "version": 99,
...
DUPLICATE_CONSTRAINT_DOC: ProviderDocument = {
    "version": 1,
    "assignments": list(VALID_DOC["assignments"]),
    "constraints": [
...
MALFORMED_CONSTRAINT_DOC: ProviderDocument = {
    "version": 1,
    "assignments": list(VALID_DOC["assignments"]),
...
# before invocation.
PARTIAL_SCOPE_DOC: ProviderDocument = {
    "version": 1,
    "assignments": [
        {
            "kind": "role",
...
UNSUPPORTED_KIND_DOC: ProviderDocument = {
    "version": 1,
...
REPEAT_SCOPE_DOC: ProviderDocument = {
    "version": 1,
    "assignments": [
        {
            "kind": "role",
...
UNKNOWN_GROUP_DOC: ProviderDocument = {
    "version": 1,
...
UNKNOWN_ENTITLEMENT_DOC: ProviderDocument = {
    "version": 1,
...
UNSUPPORTED_SCOPE_TYPE_DOC: ProviderDocument = {
    "version": 1,
    "assignments": [
        {
            "kind": "role",
...
UNKNOWN_SCOPE_ID_DOC: ProviderDocument = {
    "version": 1,
    "assignments": [
        {
            "kind": "role",
...
UNSUPPORTED_CONSTRAINT_TYPE_DOC: ProviderDocument = {
    "version": 1,
    "assignments": list(VALID_DOC["assignments"]),
    "constraints": [
        {"constraint-type": "network", "values": ["collector-a"]},
...
INVALID_CONSTRAINT_VALUE_DOC: ProviderDocument = {
    "version": 1,
    "assignments": list(VALID_DOC["assignments"]),
    "constraints": [
...
UNSUPPORTED_COMBINATION_DOC: ProviderDocument = {
    "version": 1,
    "assignments": [
...
VARIANTS: dict[str, ProviderDocument] = {
    "valid": VALID_DOC,
...
def document_bytes(variant: str) -> bytes:
    return json.dumps(copy.deepcopy(VARIANTS[variant])).encode("utf-8")


class ProviderState:
    def __init__(self, default_variant: str) -> None:
        """Initialize the mutable state that backs this fixture server."""
...
class ProviderHTTPServer(ThreadingHTTPServer):
    """HTTP server carrying the provider fixture state."""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def log_message(
        ...
        *args: object,
        ...
        sys.stderr.write(f"[fake-provider] {self.path.split('?')[0]}\n")
    ...
    def do_GET(self) -> None:
        ...
        if path == "/document":
            ...
            self.send_header("Content-Length", str(len(body)))
            self.send_header(
                "X-Fake-Provider-Calls",
                str(self.server.state.calls),
            )
            self.end_headers()
            self.wfile.write(body)
            return
        if path == "/stats":
            with self.server.state.lock:
                self._json(
                    200,
                    {
                        "calls": self.server.state.calls,
                        "variant": self.server.state.variant,
                    },
                )
            return
        if path == "/reset":
    ...
    def do_POST(self) -> None:
        ...
        if path == "/set-variant":
            ...
            if variant not in VARIANTS:
                self._json(
                    400,
                    {"error": "unknown variant", "known": sorted(VARIANTS)},
                )
                return
            with self.server.state.lock:
...
def main() -> None:
    ...
    with ProviderHTTPServer(("127.0.0.1", args.port), Handler) as httpd:
        httpd.state = state
