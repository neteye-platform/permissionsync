...
TGT_B_PORT = 19444
CLIENT = "bob"
TOKEN = "sekret-bob-token"
CLIENT_B = "alice"
TOKEN_B = "sekret-alice-token"
...
ADAPTERS = {
    ...
    "missing-import": POC / "adapters/missing-import-adapter/target/wasm32-wasip2/debug/adapter_missing_import.wasm",
    "egress": POC / "adapters/egress-adapter/target/wasm32-wasip2/release/adapter_egress.wasm",
    "wasi": POC / "adapters/wasi-adapter/target/wasm32-wasip2/release/adapter_wasi.wasm",
    "process": POC / "adapters/process-adapter/target/wasm32-wasip2/release/adapter_process.wasm",
    "runaway-mem": POC / "adapters/runaway-mem-adapter/target/wasm32-wasip2/debug/adapter_runaway_mem.wasm",
    "runaway-cpu": POC / "adapters/runaway-cpu-adapter/target/wasm32-wasip2/debug/adapter_runaway_cpu.wasm",
...
def start_provider(variant="valid"):
    return subprocess.Popen(
        [
            sys.executable, str(PROVIDER_PY),
            "--port", str(PROV_PORT), "--cert", str(CERT), "--key", str(KEY),
            "--variant", variant,
        ],
...
def start_target(delay_ms=0, port=TGT_PORT, client=None, token=None):
    ...
    return subprocess.Popen(
        [
            ...
            "--clients", f"{client}:{token}", "--delay-ms", str(delay_ms),
        ],
        stderr=subprocess.DEVNULL,
        stdout=subprocess.DEVNULL,
        start_new_session=True,
...
def reset_provider_variant(variant):
    try:
    ...
    except Exception as e:  # provider may not be up yet
        print(f"  (set-variant {variant}: {e})")


def target_state(port=TGT_PORT, client=CLIENT):
    return get_json(f"https://127.0.0.1:{port}/state?client={client}")


def reset_target():
    try:
        urllib.request.urlopen(
            f"https://127.0.0.1:{TGT_PORT}/reset", context=_ctx(), timeout=10
        )
    except Exception:
        pass


def make_config(adapter_key, deadline_ms=5000, epoch_period_ms=10,
                mem=8 * 1024 * 1024, target_endpoint=None,
                context_config=None, context_secrets=None,
...
def run_host(cfg, cwd=None):
    tmp = POC / "tests" / "_tmp_cfg.json"
    tmp.write_text(json.dumps(cfg))
    try:
        r = subprocess.run(
            [str(HOST_BIN), "run", str(tmp)],
            capture_output=True, text=True, timeout=40, cwd=cwd or str(POC),
        )
...
def make_registry(routes, **reg_overrides):
    reg = {
        "ca_cert": str(CA),
        "provider_endpoint": f"https://127.0.0.1:{PROV_PORT}",
        "deadline_ms": 5000,
...
def make_route(adapter_key, managed=True, target_endpoint=None, client=None,
               token=None, context_config=None, context_secrets=None):
    route = {
        "managed": managed,
        "target_endpoint": target_endpoint or f"https://127.0.0.1:{TGT_PORT}",
        "client": client or CLIENT,
...
def run_route(reg, key, cwd=None):
...
def ok_text(stdout, needle):
    return needle in stdout


def _snake_to_kebab(obj):
    """Recursively rewrite snake_case keys to kebab-case so the host's RECEIVED
    (serialized from the WIT record, snake_case) can be deep-compared against
    the provider's transport doc (kebab-case).""" 
    if isinstance(obj, dict):
        return {
            (k.replace("_", "-") if isinstance(k, str) else k): _snake_to_kebab(v)
...
def fetch_valid_doc():
    ...
    with urllib.request.urlopen(
        f"https://127.0.0.1:{PROV_PORT}/document", context=_ctx(), timeout=10
    ) as r:
        return json.loads(r.read().decode())
...
class Servers:
    def __init__(self, variant="valid", delay_ms=0, second_target=False):
        self.variant = variant
        self.delay = delay_ms
        self.second_target = second_target

    def __enter__(self):
        for port in (PROV_PORT, TGT_PORT):
            time.sleep(0.1)
        self.prov = start_provider(self.variant)
        self.tgt = start_target(self.delay)
        if self.second_target:
            self.tgt_b = start_target(0, port=TGT_B_PORT, client=CLIENT_B, token=TOKEN_B)
        wait_ready(self.prov, f"https://127.0.0.1:{PROV_PORT}/stats")
        wait_ready(self.tgt, f"https://127.0.0.1:{TGT_PORT}/state?client=x")
        if self.second_target:
            wait_ready(self.tgt_b, f"https://127.0.0.1:{TGT_B_PORT}/state?client=x")
        return self

    def __exit__(self, *exc):
        stop(self.prov)
        stop(self.tgt)
        if getattr(self, "tgt_b", None) is not None:
            stop(self.tgt_b)
        time.sleep(0.3)
        return False
...
def gate1_independent_lifecycle():
    ...
    with Servers("valid") as _:
        for key in ("rust", "go"):
            ...
            ok = check(
                1, f"lifecycle {key}",
                rc == 0 and ok_text(out, "RECEIVED=") and ok_text(out, "EFFECT grant"),
                f"rc={rc}",
            ) and ok
    return ok


def gate2_admission_compatibility():
    """Incompatible component rejected at admission before provider/reconcile."""
    with Servers("valid") as _:
        rc, out, err = run_host(make_config("bad"))
        admitted_bad = check(
            2, "bad adapter rejected",
            rc != 0 and "admission/instantiation failed" in err,
            f"rc={rc}",
        )
        stats = provider_stats()
        zero_calls = check(
            2, "zero provider calls on rejection",
            stats["calls"] == 0,
            f"calls={stats['calls']}",
        )
        st = target_state()
        unmanaged_intact = check(
            2, "target not mutated",
            st["managed_assignments"] == [] and st["unmanaged_assignments"] == ["legacy-admin"],
            json.dumps(st),
        )
        ...
        rc, out, err = run_host(make_config("missing-import"))
        admitted_mi = check(
            2, "missing-import adapter rejected (unprovided import)",
            rc != 0 and "admission/instantiation failed" in err,
            f"rc={rc}",
        )
        stats = provider_stats()
        zero_calls_mi = check(
            2, "missing-import => zero provider calls",
            stats["calls"] == 0,
            f"calls={stats['calls']}",
        )
        st = target_state()
        no_mutation_mi = check(
            2, "missing-import => target not mutated",
            st["managed_assignments"] == [] and st["unmanaged_assignments"] == ["legacy-admin"],
            json.dumps(st),
        )
        return admitted_bad and zero_calls and unmanaged_intact \
            and admitted_mi and zero_calls_mi and no_mutation_mi


def gate3_language_independence():
...
def gate4_model_fidelity():
    ...
    with Servers("valid") as _:
        ...
        fidelity = check(
            ...
            and ok_text(out, '"constraint_type":"network","values":["collector-a","collector-b"]')
    ...
    with Servers("unsupported-role") as _:
        for key in ("rust", "go"):
            ...
            ok = check(
                4, f"unsupported-role {key} rejected",
    ...
    with Servers("unsupported-version") as _:
        for key in ("rust", "go"):
            rc, _, err = run_host(make_config(key))
            ok = check(
                4, f"unsupported-version {key} rejected",
    ...
    with Servers("valid") as _:
        ...
        fidelity_deep = check(
            4, "full-model fidelity: RECEIVED == provider doc (deep-equal)",
    ...
    for variant, needle in semantic:
        for key in ("rust", "go"):
            with Servers(variant) as _:
                rc, out, err = run_host(make_config(key))
                rejected = check(
                    4, f"{variant} {key} rejected by adapter",
                    rc != 0 and needle in err,
                    f"rc={rc} needle={needle}",
                )
                adapter_invoked = check(
                    4, f"{variant} {key} => adapter invoked (no structural error)",
                    "admission/instantiation failed" not in err,
                    "adapter reached",
                )
                calls = provider_stats()["calls"]
                exactly_one_fetch = check(
                    4, f"{variant} {key} => exactly one provider fetch",
                    calls == 1,
                    f"calls={calls}",
                )
                st = target_state()
                no_mutation = check(
                    4, f"{variant} {key} => target not mutated",
                    st["managed_assignments"] == [],
                    json.dumps(st),
                )
                ok = ok and rejected and adapter_invoked \
                    and exactly_one_fetch and no_mutation
    ...
    for variant, needle in structural:
        with Servers(variant) as _:
            ...
            rejected = check(
                4, f"core rejects {variant} before invocation",
                rc != 0 and needle in err,
                f"rc={rc} needle={needle}",
            )
            no_invoke = check(
                4, f"{variant} => no component invocation",
                "RECEIVED=" not in out and "EFFECT grant" not in out,
                "checked stdout",
            )
            calls = provider_stats()["calls"]
            exactly_one_fetch = check(
                4, f"{variant} => exactly one provider fetch (pre-invoke)",
                calls == 1,
                f"calls={calls}",
            )
            st = target_state()
            no_mutation = check(
                4, f"{variant} => target not mutated",
                st["managed_assignments"] == [],
                json.dumps(st),
            )
...
def gate5_context_isolation():
    ...
    with Servers("valid") as _:
        rc, out, err = run_host(make_config("rust"))
        keys = check(
            5, "adapter sees keys only",
            rc == 0 and ok_text(out, "CONTEXT_KEYS=endpoint,client,token"),
            f"rc={rc}",
        )
        secret_absent = check(
            5, "secret value absent from host output",
    ...
    with Servers("valid", second_target=True) as _:
        ctx_a = {
            ...
            "context_secrets": [{"key": "api-key-a", "value": "SECRET_A_VALUE"}],
        }
        ctx_b = {
            "context_config": [{"key": "region", "value": "us-b"}],
            "context_secrets": [{"key": "api-key-b", "value": "SECRET_B_VALUE"}],
        }
        # Route A reconcile against target-a.
        rc, out, err = run_host(make_config(
            "rust", target_endpoint=f"https://127.0.0.1:{TGT_PORT}", **ctx_a))
        a_sees_own = check(
            5, "target-a adapter sees its own key + secret key",
            rc == 0 and "region" in out and "api-key-a" in out,
            f"rc={rc}",
        )
        a_not_b_key = check(
            5, "target-a adapter does not see target-b's secret key",
            "api-key-b" not in out,
            "checked stdout",
        )
        a_no_own_value = check(
            5, "target-a secret VALUE absent from host output",
        ...
        a_isolated = check(
            5, "route A does not touch target-b",
            st_a_after_a["managed_assignments"] and st_b_after_a["managed_assignments"] == [],
            json.dumps({"a": st_a_after_a["managed_assignments"], "b": st_b_after_a["managed_assignments"]}),
        )
        # Route B reconcile against target-b, using a distinct token/endpoint.
        rc, out, err = run_host(make_config(
            "rust", target_endpoint=f"https://127.0.0.1:{TGT_B_PORT}",
            client=CLIENT_B, token=TOKEN_B, **ctx_b))
        b_sees_own = check(
            5, "target-b adapter sees its own key + secret key",
            rc == 0 and "api-key-b" in out and "region" in out,
            f"rc={rc}",
        )
        b_not_a_key = check(
            5, "target-b adapter does not see target-a's secret key",
            "api-key-a" not in out,
            "checked stdout",
        )
        b_no_value = check(
            5, "target-b secret VALUE absent from host output",
            "SECRET_A_VALUE" not in out and "SECRET_B_VALUE" not in out,
            "checked stdout",
        )
        ok = ok and a_sees_own and a_not_b_key and a_no_own_value and \
            a_isolated and b_sees_own and b_not_a_key and b_no_value
    return ok


def gate6_default_deny():
    ...
    with Servers("valid") as _:
        ...
        ok = check(
            6, "egress outside allowlist denied",
        ...
        rc, out, _ = run_host(make_config("rust"))
        ok = check(
            6, "allowlisted https target succeeds",
            rc == 0 and ok_text(out, "RECEIVED="),
            f"rc={rc}",
        ) and ok
    return ok
...
def gate7_bounded_resources():
    """Runaway memory trapped by StoreLimits before host exhaustion."""
    with Servers("valid", delay_ms=0) as _:
        t0 = time.time()
        rc, out, err = run_host(make_config("runaway-mem", mem=8 * 1024 * 1024))
        elapsed = time.time() - t0
        trapped = check(
            7, "mem runaway trapped by store limits",
            rc != 0 and ("allocation" in err.lower() or "abort" in err.lower() or "oom" in err.lower()),
            f"rc={rc} elapsed={elapsed:.1f}s",
        )
        host_survived = check(7, "host process survived (not OOM-killed/exit 137)",
                              rc not in (-9, 137), f"rc={rc}")
        return trapped and host_survived


def gate8_deadline_cancellation():
    """Runaway CPU interrupted by epoch deadline; stalled outbound bounded."""
    ok = True
    # runaway-cpu: epoch trap -> no hang, error, no new work.
    with Servers("valid", delay_ms=0) as _:
        t0 = time.time()
        rc, out, err = run_host(make_config("runaway-cpu", deadline_ms=800))
        elapsed = time.time() - t0
        ok = check(
            8, "cpu runaway interrupted by deadline",
            rc != 0 and elapsed < 6.0,
            f"rc={rc} elapsed={elapsed:.1f}s",
        ) and ok
        # Do not re-fetch after trap: exactly one provider call (the pre-invoke fetch).
        stats = provider_stats()
        ok = check(
            8, "no new work after cancellation",
            stats["calls"] == 1,
            f"calls={stats['calls']}",
        ) and ok
    ...
    with Servers("valid", delay_ms=15000) as _:
        t0 = time.time()
        rc, out, err = run_host(make_config("rust", deadline_ms=500))
        elapsed = time.time() - t0
        ok = check(
            8, "stalled outbound bounded",
            rc != 0 and elapsed < 12.0,
            f"rc={rc} elapsed={elapsed:.1f}s",
        ) and ok
    return ok


def gate9_invocation_accounting():
    """Exactly one provider fetch + one reconcile, no retry."""
    with Servers("valid") as _:
        rc, out, err = run_host(make_config("rust"))
        stats = provider_stats()
        once = check(9, "exactly one provider fetch", stats["calls"] == 1,
                     f"calls={stats['calls']}")
        # Host-instrumented accounting: the reconcile was invoked exactly once
        # and the host has no retry loop (auto_retries always 0).
        account = check(
            9, "host-instrumented accounting (reconcile=1, retries=0)",
        ...
        single_reconcile = check(
            9, "single reconcile reflected in target state",
    ...
    with Servers("valid", second_target=True) as _:
        ...
        reg = make_registry({
            "unmanaged-alice": make_route(
                "rust", managed=False, target_endpoint=target_b_ep,
                client=CLIENT_B, token=TOKEN_B),
        ...
        rc, out, err = run_route(reg, "ghost-route")
        stats = provider_stats()
        ok = check(
            10, "unconfigured route -> 200/noop, no work",
            rc == 0 and ok_text(out, "ROUTE unconfigured") and stats["calls"] == 0,
            f"rc={rc} out={out} calls={stats['calls']}",
        ) and ok
        st = target_state()
        ok = check(
            10, "unconfigured route -> target untouched",
            st["managed_assignments"] == [],
            json.dumps(st),
        ) and ok
        ...
        ok = check(
            10, "another valid managed target succeeds after failures",
            rc == 0 and ok_text(out, "RECEIVED="),
            f"rc={rc}",
        ) and ok
    return ok
...
def gate11_managed_authority_statelessness():
    ...
    with Servers("valid") as _:
        ...
        st1 = target_state()
        ok = check(
            11, "managed assignments applied",
            sorted(st1["managed_assignments"]) == sorted(
                ["role:editor", "group:release-managers", "entitlement:packet-capture-download"]
            ),
            json.dumps(st1["managed_assignments"]),
        ) and ok
        ok = check(
            11, "unmanaged state untouched",
            st1["unmanaged_assignments"] == ["legacy-admin"]
            and st1["unmanaged_constraints"] == {"legacy": ["keep"]},
            json.dumps({"u_ass": st1["unmanaged_assignments"],
        ...
        st2 = target_state()
        ok = check(
            11, "restart succeeds idempotently",
...
GATES = [
    ...
    (11, "Managed authority & statelessness", gate11_managed_authority_statelessness),
]


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--only", type=int, nargs="*", help="only run these gate numbers")
    args = ap.parse_args()

    if not HOST_BIN.exists():
        print(f"FATAL: host binary not built at {HOST_BIN}. Run `make build` first.")
        return 2
    ...
    print()
    verdict = "ALL 11 GATES PASS" if all_pass else f"NOT ALL PASS ({sum(1 for v in gate_ok.values() if v)}/11)"
