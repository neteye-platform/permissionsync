...
pub struct GenericRestPermissionProviderConfig {
    ...
    /// PEM-encoded private trust anchors added to the platform system roots.
    ///
    /// Each element is a PEM certificate bundle and may contain one or more
    /// concatenated X.509 certificates.
...
impl GenericRestPermissionProvider {
    ...
    /// empty and, when present, only adds private trust anchors parsed from
    ...
    pub fn new(
        ...
        let tls_connector = client::build_tls_connector(&config.additional_trust_anchors_pem)?;
