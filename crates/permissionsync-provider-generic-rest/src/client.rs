...
///
/// System trust roots remain enabled. `additional_trust_anchors_pem` may be
/// empty and, when present, each element is parsed as a PEM certificate
/// bundle that may contain one or more concatenated X.509 certificates; every
/// certificate found is added as an additional private trust anchor. A PEM
/// blob that is malformed or that yields no usable certificate fails
/// construction. This connector carries no connection pool, executor, or
/// resolver: each Provider operation performs its own single-attempt DNS
/// resolution, TCP connect, and TLS and HTTP/1.1 handshake, as required by
/// ADR 0008.
        ...
        EndpointHostKind::Hostname { name, name_server } => {
            ...
            std::future::poll_fn(move |task_context| {
                ...
                if ipv4_done && ipv6_done {
                    // Neither family produced a usable address. A DNS
                    // timeout is reported as DeadlineExceeded when at least
                    // one family ended because of timeout exhaustion. Only
                    // an aggregate containing no timeout is a Transport
                    // failure. The underlying Hickory diagnostics are
...
/// Classifies a completed A/AAAA resolution in which neither family produced
/// a usable address. Any genuine Hickory timeout determines the aggregate
/// category; otherwise the aggregate is a transport failure.
fn classify_dns_no_address(ipv4_timed_out: bool, ipv6_timed_out: bool) -> ProviderFailure {
...
mod tests {
    ...
    use super::{
        ...
        accumulate_within_limit, authorization_header, classify_dns_no_address, endpoint_dns_name,
        is_hickory_timeout, parse_endpoint, resolve_one_address,
    };
    ...
    async fn dns_timeout_has_no_retransmit_or_residual_worker_datagram() {
        ...
        assert!(
            matches!(result, Err(ProviderFailure::DeadlineExceeded)),
            "a Hickory DNS timeout must classify as DeadlineExceeded, not Transport"
        );
    }

    #[test]
    fn only_a_hickory_timeout_error_classifies_as_timeout() {
        assert!(is_hickory_timeout(&NetError::Timeout));
        assert!(!is_hickory_timeout(&NetError::Message(
            "sentinel-non-timeout-dns-failure"
        )));
    }

    #[test]
    fn no_address_with_one_timeout_and_one_non_timeout_failure_is_deadline_exceeded() {
        let timed_out = is_hickory_timeout(&NetError::Timeout);
        let non_timeout =
            is_hickory_timeout(&NetError::Message("sentinel-non-timeout-dns-failure"));

        assert!(matches!(
            classify_dns_no_address(timed_out, non_timeout),
            ProviderFailure::DeadlineExceeded
        ));
    }

    #[test]
    fn no_address_with_only_non_timeout_failures_is_transport() {
        let non_timeout =
            is_hickory_timeout(&NetError::Message("sentinel-non-timeout-dns-failure"));

        assert!(matches!(
            classify_dns_no_address(non_timeout, non_timeout),
            ProviderFailure::Transport
        ));
    }

    #[tokio::test]
    async fn a_usable_address_from_one_family_succeeds_despite_the_other_familys_dns_failure() {
        let socket = UdpSocket::bind("127.0.0.1:0")
            .await
            .expect("local DNS endpoint must bind");
        let endpoint = hostname_endpoint("provider.test", socket.local_addr().expect("address"));
        ...
        // Answer only the A query with a valid response and let the AAAA
        // query time out: the operation must still succeed using the usable
        // IPv4 address rather than failing because the other family failed.
        ...
        let (result, ()) = tokio::join!(resolve_one_address(&endpoint, deadline), server);

        assert_eq!(
            result.expect("the successful A response must resolve despite the AAAA failure"),
            "192.0.2.44:443".parse().expect("socket address")
        );
    }
