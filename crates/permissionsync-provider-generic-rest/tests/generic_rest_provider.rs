...
#[test]
fn rejects_malformed_trust_anchor_pem_bytes() {
    let mut config = test_config("https://127.0.0.1/permissions");
    config.additional_trust_anchors_pem = vec![vec![0, 1, 2, 3]];
...
fn rejects_a_pem_blob_with_no_usable_certificate() {
    let mut config = test_config("https://127.0.0.1/permissions");
    // No PEM start line at all: `Certificate::stack_from_pem` accepts this
    // and returns an empty certificate list rather than an error, so it must
    // still be explicitly rejected, since every configured PEM blob must
    // contribute at least one certificate.
    config.additional_trust_anchors_pem = vec![b"not a pem certificate\n".to_vec()];

    assert!(GenericRestPermissionProvider::new(config).is_err());
}

#[test]
fn accepts_empty_additional_trust_anchors() {
    let config = test_config("https://127.0.0.1/permissions");

    assert!(GenericRestPermissionProvider::new(config).is_ok());
}
...
async fn fails_when_the_provider_operation_timeout_elapses() {
    ...
    config.additional_trust_anchors_pem = vec![identity.trust_anchor_pem];
...
async fn cancellation_after_the_request_is_sent_is_observed_before_reading_the_response() {
    ...
    let port = local_port(&listener);
    let identity = build_primary_test_identity(LOOPBACK_ADDRESS);
    let trust_anchor = identity.trust_anchor_pem.clone();
    let acceptor = TlsAcceptor::from(identity.acceptor);
    ...
    let mut config = test_config(&format!("https://{LOOPBACK_ADDRESS}:{port}/permissions"));
    config.additional_trust_anchors_pem = vec![trust_anchor];
    let provider = GenericRestPermissionProvider::new(config).expect("valid configuration");
...
#[tokio::test]
async fn accepts_a_pem_bundle_containing_the_needed_ca_among_multiple_certificates() {
    let listener = bind_loopback_listener().await;
    let port = local_port(&listener);
    let identity = build_primary_test_identity(LOOPBACK_ADDRESS);
    let acceptor = TlsAcceptor::from(identity.acceptor);

    // An unrelated CA is concatenated with the CA the server chain actually
    // needs, exercising a multi-certificate PEM bundle rather than a single
    // certificate.
    let unrelated_trust_anchor_pem = build_test_identity(
        LOOPBACK_ADDRESS,
        ALTERNATE_ROOT_KEY_LABEL,
        ALTERNATE_LEAF_KEY_LABEL,
    )
    .trust_anchor_pem;
    let mut trust_anchor_bundle_pem = unrelated_trust_anchor_pem;
    trust_anchor_bundle_pem.extend_from_slice(&identity.trust_anchor_pem);

    let response = ok_response(br#"{"version":1,"payload":null}"#);
    let server = tokio::spawn(serve_one_request(listener, acceptor, response));

    let mut config = test_config(&format!("https://{LOOPBACK_ADDRESS}:{port}/permissions"));
    config.additional_trust_anchors_pem = vec![trust_anchor_bundle_pem];
    let provider = GenericRestPermissionProvider::new(config).expect("valid configuration");
...
async fn rejects_a_server_certificate_signed_by_an_untrusted_root() {
    ...
    let mut config = test_config(&format!("https://{LOOPBACK_ADDRESS}:{port}/permissions"));
    config.additional_trust_anchors_pem = vec![unrelated_trust_anchor];
    let provider = GenericRestPermissionProvider::new(config).expect("valid configuration");
...
async fn rejects_a_streamed_body_that_exceeds_the_response_body_limit() {
    ...
    let server = tokio::spawn(async move {
        ...
        // The client is expected to stop reading once the response-body
        // ceiling is exceeded and may close while this oversized response is
        // still being written.
        let _ = stream.write_all(&response).await;
        let _ = stream.shutdown().await;
...
fn build_test_identity(ip_address: &str, root_label: &str, leaf_label: &str) -> TestIdentity {
    ...
    TestIdentity {
        trust_anchor_pem: root_certificate.to_pem().expect("root certificate pem"),
        acceptor,
    }
}
...
fn build_root_certificate(label: &str) -> (X509, PKey<Private>) {
    ...
    // Each root uses a subject name derived from its own label so that two
    // independent deterministic roots (for example the primary root and the
    // alternate root used to build an unrelated trust anchor) never share a
    // subject name, which would otherwise make chain building ambiguous when
    // both are trusted at once, such as in a multi-certificate PEM bundle.
    let name = build_name(&format!("permissionsync-test-root-{label}"));
